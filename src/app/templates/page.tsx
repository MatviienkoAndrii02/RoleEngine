    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{t("template.listSubtitle")}</p>
        </div>
        <Button asChild><Link href="/templates/new">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {templates.length === 0 ? (
          <Card>
            <CardHeader>
              <CardTitle>{t("template.noTemplatesTitle")}</CardTitle>
            </CardHeader>
            <CardContent className="text-sm text-muted-foreground">
              {t("template.noTemplatesBody")}
            </CardContent>
          </Card>
        ) : (
